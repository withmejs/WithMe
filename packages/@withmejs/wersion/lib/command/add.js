...
export function add() {
  ;(async function () {
    ...
    await enquirer.prompt({
      type: 'MultiSelect',
      name: 'pkgs_major',
      message: `👻 Please select some packages that will have a ${red(
        'major'
      )} bump`,
    ...
    await enquirer.prompt({
      ...
      name: 'pkgs_minor',
      message: `👻 Please select some packages that will have a ${green(
        'minor'
      )} bump`,
    ...
    await enquirer.prompt({
      ...
      name: 'pkgs_patch',
      message: `👻 Please select some packages that will have a ${blue(
        'patch'
      )} bump`,
    ...
    await enquirer.prompt({
      ...
      message: `👻 Please select some packages that will have a ${yellow(
        'prerelease'
      )} bump`,
    ...
    await enquirer.prompt({
      type: () => {
        if (
          enquirer.answers.pkgs_prerelease !== undefined &&
          enquirer.answers.pkgs_prerelease.length !== 0
        ) {
          return 'form'
        } else {
          return
    ...
    if (flag) {
      ...
      for (const key in enquirer.answers) {
        ...
        if (enquirer.answers[key].length !== 0 && key !== 'pkgs_selected') {
          addPkgs[changeName(key)] = enquirer.answers[key]
        }
      }

      shell.cd(wersionDir)
      shell.exec(`echo '${JSON.stringify(addPkgs)}' > .wersionrc.json`)
    } else {
